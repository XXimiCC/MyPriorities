...
describe('неделя против предыдущей', () => {
  ...
  /**
   * Граница проходит ровно по первой записи: окно prev7 — это дни с 13-го по
   * 7-й назад, и пока история начинается позже 13-го дня, часть предыдущей
   * недели человек прожил без приложения. Блоков в окне при этом хватает с
   * запасом — молчание даёт именно длина истории, а не порог по блокам.
   */
  it('неполная предыдущая неделя молчит, хотя блоков в окне хватает', () => {
    const journal = empty();
    // Первая запись — 12 дней назад: самого старого дня окна в журнале нет.
    for (let i = 7; i <= 12; i += 1) journal.clicks[back(i)] = { ab: 3 };
    for (let i = 0; i <= 6; i += 1) journal.clicks[back(i)] = { ab: 1 };
    expect(run(journal)).not.toContain('weekOverWeek');
  });

  it('на первом же дне, когда окно покрыто целиком, сравнение появляется', () => {
    const journal = empty();
    // Тот же профиль, что и выше, плюс один день: история дотянулась до 13-го.
    for (let i = 7; i <= 13; i += 1) journal.clicks[back(i)] = { ab: 3 };
    for (let i = 0; i <= 6; i += 1) journal.clicks[back(i)] = { ab: 1 };
    expect(run(journal)).toContain('weekOverWeek');
  });
});
