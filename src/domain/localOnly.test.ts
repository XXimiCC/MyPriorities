...
import { DEFAULT_MODULES, type DayKey, type Journal, type Settings } from './types';
...
function since(days: number): Journal {
  return { clicks: { [back(days)]: { ab: 1 } }, battery: {} };
}

describe('возраст истории', () => {
  it('пустой журнал — ноль дней', () => {
    expect(historyDays({ clicks: {}, battery: {} }, NOW)).toBe(0);
  });
  ...
  it('отметка заряда — тоже история', () => {
    expect(historyDays({ clicks: {}, battery: { [back(40)]: [[600, 2]] } }, NOW)).toBe(41);
  });
});
