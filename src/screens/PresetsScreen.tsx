          ...
          {PRESETS.map((preset) => (
            <li key={preset.id}>
              ...
              >
                ...
                {/*
                  Что человек получит, если возьмёт этот набор, — прямо на
                  карточке и в том же порядке, в каком приоритеты встанут на
                  главной. Типаж одной строкой («Тело как главный проект»)
                  говорит, про кого набор, но не про то, что появится на
                  экране; названия говорят.
                */}
                <span className="pcard__list">
                  {preset.priorities.map((item) => (
                    <span
                      key={item.titleKey}
                      className="pcard__item"
                      style={{ '--accent': colorOf(item.colorId).hex } as React.CSSProperties}
                    >
                      <span className="swatch pcard__dot" />
                      {t(item.titleKey)}
                    </span>
                  ))}
                </span>
