 ...
 * совпадает с сидом демо-профиля «Артём» (src/demo/profiles.ts), а пятница даёт
 * в тридцатидневном окне достаточно и будних, и выходных дней — иначе наблюдение
...
async function openApp(context, url) {
  ...
  /*
   * Язык прибит параметром, а не оставлен на усмотрение контекста. Локаторы
   * ищут русские строки из ru.ts, а язык приложение выбирает из четырёх
   * источников (src/i18n/index.ts); совпадение с ru-RU в контексте и стабом
   * Telegram — везение, а не договор. Съёмка на чужом языке упала бы на
   * ненайденном локаторе через двадцать секунд ожидания.
   *
   * Прогон вправе попросить другой язык адресом — так снимается английский
   * постер лендинга. Умолчание при этом остаётся русским для всех остальных.
   */
