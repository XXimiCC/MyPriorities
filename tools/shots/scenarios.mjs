 ...
 *
 * Четыре прогона, потому что четыре состояния приложения несовместимы в одном:
 *   demo  — ?mock=1, профиль «Артём»: заполненные экраны, шторки, лимиты;
 *   empty — чистый localStorage: онбординг и все пустые состояния
 *           (в демо onboarded = true, туда просто не попасть);
...
const demo = [
  ...
  {
    name: 'demo-list',
    note: 'Витрина демо-профилей: пять чужих жизней на выбор',
    setup: async (page) => {
      await goTab(page, 'tab.settings');
      await page.getByRole('button', { name: s('demo.settingsRow') }).click();
      await page.locator('.demos').waitFor();
    },
    tall: true,
  },
...
// --- Прогон D: гостевой режим --------------------------------------------

/*
 * `?demo=` — то, что открывает друг: плашка сверху и настройки без всего, что
 * трогает чужой аккаунт. `?mock=` этого не включает специально, иначе снять
 * обычные настройки было бы нечем.
 */
const guest = [
  {
    name: 'demo-bar',
    note: 'Плашка гостевого режима: чьи данные на экране и как вернуться к своим',
    setup: () => Promise.resolve(),
    target: '.demobar',
  },
  {
    name: 'demo-max-home',
    note: 'Профиль «Максимум»: десять приоритетов и тринадцать месяцев за спиной',
    setup: () => Promise.resolve(),
  },
  {
    name: 'demo-settings',
    note: 'Настройки в гостевом режиме: ни входа, ни копии, ни сброса',
    setup: (page) => goTab(page, 'tab.settings'),
    scrollTo: '.sset__facts',
  },
];
...
// --- Прогон F: постер для лендинга ----------------------------------------

/*
 * Единственный кадр, снятый не в телефонный экран.
 *
 * Он лежит постером под живым фреймом на лендинге и обязан совпасть с ним
 * во всём: тот же профиль (`?mock=max`, как у фрейма) и та же высота
 * (FRAME_HEIGHT ниже = высота рамки в landing/src/landing.css). Разойдётся
 * профиль — при подмене поедут числа и число полос; разойдётся высота — в
 * постер не попадёт панель вкладок, и в момент подмены она «появится».
 */
const FRAME = { width: 390, height: 720 };

const framed = [
  {
    name: 'home-frame',
    note: 'Постер под живым фреймом лендинга: «Максимум» в высоту рамки',
    setup: () => Promise.resolve(),
  },
];
